...
const char *const html =
    ...
    "href=\"data:image/png;base64,iVBORw0KGgo=\" /><style>/*! tailwindcss v4.1.6 | MIT License | "
    ...
    "</h2><h3 id=\"timer\" class=\"text-sm font-medium text-zinc-500\"> Секунд до оновлення: 10 </h3></div><div "
    "class=\"container mx-auto p-4 lg:p-8 xl:max-w-7xl grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-12 "
    "lg:gap-8\" ><div class=\"rounded-lg border border-zinc-200 bg-white hover:bg-zinc-50/50 active:border-purple-200 "
    "lg:col-span-3 flex grow items-center justify-between p-5\" ><dl><dt id=\"temperature\" class=\"text-2xl "
    "font-bold\">...</dt><dd class=\"text-sm font-medium text-zinc-500\">Температура</dd></dl><div class=\"flex "
    ...
    "1-7.5 0 3.75 3.75 0 0 1 7.5 0Z\" /></svg></div></div></div></main></div><script> let secondsLeft = 10; let "
    "interval = null; async function updateSensorData() { const { dht11: { t, h }, l, } = await (await "
    "fetch(\"/api\")).json(); document.getElementById(\"temperature\").innerHTML = `${t}°C`; "
    "document.getElementById(\"humidity\").innerHTML = `${h}%`; document.getElementById(\"luminosity\").innerHTML = "
    "`${l}/4095`; } async function handleTimer() { secondsLeft--; const timerElement = "
    "document.getElementById(\"timer\"); timerElement.innerHTML = `Секунд до оновлення: ${secondsLeft}`; if "
    "(secondsLeft <= 0) { clearInterval(interval); timerElement.innerHTML = `Оновлення...`; await updateSensorData(); "
    "secondsLeft = 10; interval = setInterval(handleTimer, 1000); } } document.addEventListener(\"DOMContentLoaded\", "
    "() => { setTimeout(async () => { await updateSensorData(); interval = setInterval(handleTimer, 1000); }, 1000); "
    "}); </script></body></html>";